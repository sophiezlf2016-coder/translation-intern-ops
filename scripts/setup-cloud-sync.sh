...
echo ""
echo "方式 A — 提交到 GitHub（推荐，同事自动生效）："
echo "  1. 打开仓库 Settings → Secrets and variables → Actions"
echo "  2. 添加 SUPABASE_URL 和 SUPABASE_ANON_KEY"
echo "  3. git add sync-config.js && git commit && git push"
echo ""
echo "方式 B — 仅本机：push 后同事需各自运行本脚本"
echo ""
echo "然后把链接发给同事，由你首次设置团队密码，同事用同一密码登录。"
